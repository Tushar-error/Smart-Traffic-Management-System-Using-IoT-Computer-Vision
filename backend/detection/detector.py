...
from config import (
    ...
    DIRECTION_REGIONS, FRAME_WIDTH, FRAME_HEIGHT, NORTH, EAST, SOUTH, WEST
)
...
class VehicleDetector:
    ...
    def detect(self, frame):
        ...
        detections = []
        emergency_detected = {NORTH: False, EAST: False, SOUTH: False, WEST: False}
        ...
        for r in results:
            for box in r.boxes:
                ...
                # Prototype: If it's a truck (7) or bus (5), mark as potential emergency for priority demo
                is_emergency = cls in [5, 7] and conf > 0.6
                if is_emergency:
                    emergency_detected[direction] = True
                ...
                color = (0, 0, 255) if is_emergency else self._direction_color(direction)
                cv2.rectangle(frame, (x1, y1), (x2, y2), color, 3 if is_emergency else 2)
                
                label = f"PRIORITY" if is_emergency else {NORTH: "N", EAST: "E", SOUTH: "S", WEST: "W"}[direction]
                cv2.putText(frame, f"{label} {conf:.0%}", (x1, y1 - 5),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.45, color, 1)

        self._draw_overlay(frame, direction_counts)
        return frame, direction_counts, emergency_detected
